...
Polymer({
	_template: html`
		...
		<template is="dom-if" if="[[!richTextEnabled]]">
			<d2l-input-textarea id="textEditor" hidden$="[[richTextEnabled]]" aria-invalid="[[ariaInvalid]]" aria-label="[[ariaLabel]]" disabled="[[disabled]]" no-border="" hover-styles="" value="[[value]]" on-change="_onInputChange"></d2l-input-textarea>
		</template>
		<template is="dom-if" if="[[richTextEnabled]]">
